...
log-posterior :<br>$
\ln p(U,V \mid R,\sigma^2,\sigma_V^2,\sigma_U^2) = -\frac{1}{2\sigma^2}\sum_{i=1}^N\sum_{j=1}^M\mathrm{I_{i,j}}(R_{i,j}-{U_{i}^T}V_j)^2<br>-\frac{1}{2\sigma_U^2}{\sum_{i=1}^N}{U_{i}^T}U_i -\frac{1}{2\sigma_V^2}{\sum_{j=1}^M}{V_{j}^T}V_j <br>-\frac{1}{2}((\sum_{i=1}^N\sum_{j=1}^M\mathrm{I_{i,j}})\ln\sigma^2 +\mathrm{ND}\ln\sigma_U^2 + \mathrm{MD}\ln\sigma_V^2) + \mathrm{C}
$<br>
